...
3. 기간 조회·파기 기준은 **일(日) 단위(`collectedDate`)** 를 사용한다. 관리자 열람용 **수집 일시(`collectedAt`, KST 분 단위)** 를 별도 저장·표시한다(운영 협의 반영 — 응답은 완전 익명이라 개인 연결 정보는 없음, 관리자 전용 열람).
4. 중복응답 방지는 기기별 1회(localStorage) 방식(운영 협의 — 강의실 공유 URL 특성상 토큰 미발급). 응답 문서에 개인 식별정보를 저장하지 않는다.

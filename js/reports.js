...
import {
  computeAgg, deserializeAgg, renderEduHTML, renderInstHTML, renderOxHTML, renderExtraHTML, renderChoiceHTML, renderDatesHTML, EDU_ITEMS,
} from "./agg.js";
...
// 추가 카테고리·O/X·선택형·날짜 표: 응답이 있을 때만 섹션을 노출한다.
function renderOptional(agg) {
  ...
  const datesHtml = agg ? renderDatesHTML(agg) : "";
  ...
  document.getElementById("rep-dates-wrap").hidden = !datesHtml;
  document.getElementById("rep-dates").innerHTML = datesHtml;
}
